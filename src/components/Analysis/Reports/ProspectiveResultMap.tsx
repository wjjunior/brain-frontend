...
interface IReportProspectiveResultMapComponentProps {
  patientName: string;
  coreSetName: string;
  fillDate: string;
  categories: Array<{
...
const ReportProspectiveResultMapComponent: React.FC<IReportProspectiveResultMapComponentProps> = ({
  patientName,
  coreSetName,
  fillDate,
  categories,
    ...
    <Flex direction="column">
        ...
        <Breadcrumb>
          ...
          <Breadcrumb.Item
            text={intl.formatMessage({
              id: 'analysis.prospectiveResultMap.reportTitle',
            })}
          />
        </Breadcrumb>
        <H6 style={{ marginTop: grid(1) }}>
          {intl.formatMessage({
            id: 'analysis.prospectiveResultMap.reportDescription',
          })}
        </H6>
      </Flex>
      <Card flex="1" style={{ padding: grid(4) }}>
        <H4 color="gray" bold style={{ marginBottom: grid(3) }}>
          {intl.formatMessage({
            id: 'analysis.prospectiveResultMap.reportResultTitle',
          })}
        </H4>
        <Flex flex="1">
          ...
          <Flex flex="1" direction="column">
            <H6 bold color="gray" style={{ marginBottom: grid(2) }}>
              {intl.formatMessage({
                id: 'common.analysis.prospectiveResultMap',
              })}
            </H6>
        ...
        <H6 color="gray" style={{ marginBottom: grid(1) }}>
          {intl.formatMessage({
            id: 'analysis.prospectiveResultMap.reportEasierWithFilters',
          })}
        </H6>
        <H6 bold color="gray" style={{ marginBottom: grid(2) }}>
          {intl.formatMessage({
            id: 'analysis.prospectiveResultMap.reportFilterLabel',
          })}
        </H6>
        <Select
          placeholder=""
...
export default ReportProspectiveResultMapComponent;
