...
import { actions } from '../../../modules/analysis';
import {
...
interface IPatientGlobalPrognosticsAnalysisProps {}

const PatientGlobalPrognosticsAnalysis: React.FC<IPatientGlobalPrognosticsAnalysisProps> = () => {
  const intl = useIntl();
  ...
  const tableRows: Row[] = focusedPatientDiagnostics.map((row) => ({
    ...
    options: (
      <RowOptions
        diagnosticReportUrl={ROUTE_ANALYSIS_REPORT_GLOBAL_PROGNOSTIC.replace(
          ':patientId',
          patientId
        )
  ...
  const onButtonClick = () =>
    navigate(
      ROUTE_ANALYSIS_CREATE_GLOBAL_PROGNOSTIC + `?patientId=${patientId}`
    );

  return (
    <SharedPatientAnalysisComponent
      title={intl.formatMessage({ id: 'patients.globalPrognosticsTitle' })}
...
export default PatientGlobalPrognosticsAnalysis;
