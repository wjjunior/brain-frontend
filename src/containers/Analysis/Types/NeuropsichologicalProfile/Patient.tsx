...
// import { updateFilledNeuropsichologicalProfileDraft } from '../../../../modules/analysis/actions';
import { ROUTE_ANALYSIS_NEUROPSICHOLOGICAL_PROFILE_INTELECTUAL_OPERATION } from '../../../../modules/analysis/constants';
...
import {
  ...
  selectNeuropsichologicalProfilePatient,
...
const NeuropsichologicalProfilePatientContainer: React.FC = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const patientsList = useSelector(selectors.patientsList);
  const draftPatientId = useSelector(
    analysisSelectors.draftNeuropsichologicalProfilePatientId
  );
  ...
  const updateSelectedPatient = useCallback(
    function (options: Option[]) {
      ...
      dispatch(selectNeuropsichologicalProfilePatient(option.key));
    },
    [dispatch]
  );
  ...
  function onNextClick() {
    if (selectedPatient) {
      // initialize draft with selectedPatient.key
      dispatch(selectNeuropsichologicalProfilePatient(selectedPatient.key));
      dispatch(markNeuropsichologicalProfileProgress('patient'));
...
export default NeuropsichologicalProfilePatientContainer;
