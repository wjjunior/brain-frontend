...
// import { updateFilledGlobalPrognosticDraft } from '../../../../modules/analysis/actions';
import {
  DocumentTypes,
...
import {
  ...
  selectGlobalPrognosticCoreSet,
  selectGlobalPrognosticPatient,
...
const GlobalPrognosticPatientContainer: React.FC = () => {
  const intl = useIntl();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const patientsList = useSelector(selectors.patientsList);
  const draftPatientId = useSelector(
    analysisSelectors.draftGlobalPrognosticPatientId
  );
  ...
  const updateSelectedPatient = useCallback(
    function (options: Option[]) {
      ...
      dispatch(selectGlobalPrognosticPatient(option.key));
      dispatch(
  ...
  function onNextClick() {
    if (selectedPatient && selectedCoreSet) {
      // initialize draft with selectedPatient.key
      dispatch(selectGlobalPrognosticPatient(selectedPatient.key));
      const [coreSetId, diagnosticId] = selectedCoreSet.key.split(':');
      dispatch(selectGlobalPrognosticCoreSet(coreSetId, diagnosticId));
      dispatch(markGlobalPrognosticProgress('patient'));
...
export default GlobalPrognosticPatientContainer;
